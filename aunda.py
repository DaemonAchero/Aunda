                    ...
                    page.evaluate("""({ dir }) => {
                        ...
                        if (isYouTube) {
                            ...
                            function smoothStep(timestamp) {
                                ...
                                window.scrollBy(0, (scrollDistance / duration) * 16 * easeOut);
                            ...
                            const scrollDistance = 650 * dir;
                            const tiktokContainer = document.querySelector('[class*="-SetItemContainer"]') ||
                                                    document.querySelector('#column-list-container') ||
                                                    document.querySelector('main');

                            if (tiktokContainer) {
                                tiktokContainer.scrollBy({ top: scrollDistance, behavior: 'smooth' });
                            } else {
                                window.scrollBy({ top: scrollDistance, behavior: 'smooth' });
                            }

                        } else {
                            window.scrollBy({ top: 300 * dir, behavior: 'smooth' });
                        }
                    }""", {"dir": direction})
...
# --- Responsive Swipe Control Variables ---
prev_swipe_y = None
swipe_latched = False
...
# Very gentle motion threshold for instant response
EASY_SWIPE_TRIGGER_DIST = 0.012 
...
def is_two_finger_up_pose(lm):
    # Ultra-relaxed check: Index & Middle tips are simply above their PIP joint (knuckle)
...
def draw_ui_compact_left(img, active_zone, locked, is_pt_active, active_pt, is_reversed):
    ...
    if is_top:
        ...
        txt = "TOP (FIST=LOCK | PINCH=UNLOCK)" if locked else "TOP"
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            hand_scale_3d = dist_3d(lm[0], lm[5]) or 0.1
            hand_scale_2d = dist_2d(lm[5], lm[17]) or 0.1
            ...
            if is_fist:
                if not fist_latched:
                    ...
                    elif is_locked and current_zone != "top":
                        is_locked = False
                        last_unlock_time = time.time()
                        reset_all_subfeatures()
                        fist_latched = True
            ...
            # --- ZONE INTERACTIONS & PINCH CONTROLLER ---
            if is_locked:
                if current_zone == "top":
                    if index_near_thumb:
                        if not index_pinch_latched and not in_cooldown:
                            is_locked = False
                            last_unlock_time = time.time()
                            reset_all_subfeatures()
                    ...
                    if is_two_finger_up_pose(lm):
                        ...
                        if prev_swipe_y is not None:
                            ...
                            # Trigger as soon as the upward motion exceeds threshold
                            if delta_y > EASY_SWIPE_TRIGGER_DIST and not swipe_latched:
                                cmd = "SWIPE_REVERSED" if middle_reversed_active else "SWIPE_DEFAULT"
                                send_command(cmd, "SCROLL_DOM")
                                swipe_latched = True
                            
                            # Reset latch when fingers slow down or stop moving up
                            elif delta_y < 0.002:
                                swipe_latched = False
